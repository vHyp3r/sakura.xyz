...
const firebaseFallbackConfig = {
  ...
  // Use the exact value from Firebase Console (keep SDK config identical)
...
// Some REST endpoints (Firebase Storage v0) expect the bucket host to be
// the appspot.com hostname. Provide a computed REST bucket name so callers
// can use the correct host when issuing raw HTTP uploads.
const storageBucketForREST = (firebaseConfig.storageBucket || '').replace(/\.firebasestorage\.app$/i, '.appspot.com');
...
export { db, storage, storageBucketForREST };
