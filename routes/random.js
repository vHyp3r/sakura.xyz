...
const mockPacks = [
    ...
    { id: 5, name: "Ultimate PvP Bundle", resolution: "64x", category: "PvP", downloads: 3200, uploader: "PvPMaster", thumbnail: "/images/default-thumbnail.svg", isPort: true }
];

// Random page - redirect to a random pack
router.get('/', (req, res) => {
    const randomIndex = Math.floor(Math.random() * mockPacks.length);
    const randomPack = mockPacks[randomIndex];
    res.redirect(`/packs/${randomPack.id}`);
});
